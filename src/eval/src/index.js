...
class Daemon extends EventEmitter {
  ...
  _startElectron (opts, cb) {
    ...
    console.log("Check value: " + opts.electron);
    // this.child = spawn(path.resolve("D:\\Project\\TND\\rsb-torrent-client\\node_modules\\electron\\dist\\electron.exe"), [ opts.daemonMain ], electronOpts)
    this.child = spawn(opts.electron, [ opts.daemonMain ], electronOpts)
    this.child.on('close', (code) => {
      if (this.closing) return
      var err = `electron-eval2 error: Electron process exited with code ${code}`
      if (exitStderr) err += `.\nStderr:\n${exitStderr}`
